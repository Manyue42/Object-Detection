...
dir_path = 'C:/Users/Manyue/Desktop/ObjDetect/'
...
folders_path = os.path.join(dir_path, 'Annotations', '*')
...
for folder in folders:
    ...
    path = path.replace('\\', '/')
    label_path = folder.replace('Annotations', 'labels').replace('.xml', '.txt').replace('\\', '/')
    ...
    for obj in root.iter('object'):
        ...
        width = abs(b[0] - b[1])
        height = abs(b[2] - b[3])
        img_path = folder.replace("Annotations", "JPEGImage").replace('xml', 'jpg').replace('\\', '/')
